...
export async function generateMetadata({
  ...
  if (!clock) return {};
  const days = Math.floor((Date.now() - clock.startedAt.getTime()) / (1000 * 60 * 60 * 24));
  const status = clock.status === "resolved" ? "Resolved" : `${days} days and counting`;
  return {
    title: clock.subjectLine,
    description: `${status} — waiting on ${clock.recipientEmail}. Part of the ${clock.campaign.name} campaign.`,
    openGraph: {
      title: `${clock.subjectLine} — clock.email`,
      description: `${status}. ${clock.recipientEmail} has ${clock.status === "resolved" ? "responded" : "not responded"}.`,
    },
  };
}
...
export default async function ClockPage({
  ...
  return (
    <main className="max-w-2xl mx-auto px-6 py-10">
      ...
      {/* Timer hero */}
      <div className="mt-6 bg-velvet-700 rounded-xl px-5 sm:px-8 py-6 sm:py-8">
        <h1 className="text-2xl sm:text-3xl font-bold text-white">{clock.subjectLine}</h1>
        <div className="mt-5 pt-5 border-t border-velvet-500">
          <p className="text-xs text-velvet-100 uppercase tracking-wide mb-2">
            {clock.status === "resolved" ? "Waited" : "Waiting"} for a response from{" "}
            <Link
              href={`/recipient/${encodeURIComponent(clock.recipientEmail)}`}
              className="text-white font-medium normal-case hover:text-velvet-100 transition-colors"
            >
